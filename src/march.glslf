...
float distScene(vec3 p) {
    ...
    float box = sdBox(q - vec3(0.0, 1.0, 0.0), vec3(0.2));
    p.x = mod(p.x, 2.0);
    float sphere = sdSphere(p - vec3(1.0, 1.0, 2.0), 0.5);
...
void main() {
    vec2 uv = vec2(gl_FragCoord.x / g_windowWidth, gl_FragCoord.y / g_windowHeight);
